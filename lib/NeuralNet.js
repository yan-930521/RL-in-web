class NeuralNet {
  ...
  update(inputs) {
    ...
    for (let k = 0; k < this.no; k++) {
      ...
      // 真正的數字要經過 sigmoid 處理
      this.ao[k] = this.sigmoid(sum, "tanh")*10;
      //this.ao[k] = sum;
    }
    return this.ao;
  }
  ...
  backPropagate(targets, rate, moment) {
    ...
    for (let k = 0; k < this.no; k++) {
      let error = targets[k] - this.ao[k]/10;
      output_deltas[k] = this.dsigmoid(this.ao[k]/10, "tanh") * error;
      //output_deltas[k] = this.ao[k] * error;
    }
  ...
  check(patterns) {
    for (let p in patterns) {
        ...
        let outputs= patterns[p][1];
        console.log(outputs+" / "+this.update(inputs))
    }
}
